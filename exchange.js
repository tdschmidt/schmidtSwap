...
const exchange_name = 'schmidtSwap';             // TODO: fill in the name of your exchange

const token_name = 'schmidtcoin';                // TODO: replace with name of your token
const token_symbol = 'SCHM';              // TODO: replace with symbol for your token


// TODO: Paste token and exchange contract ABIs in abi.js
...
/*** ADD LIQUIDITY ***/
let decimilization = 10000000

async function addLiquidity(amountEth, maxSlippagePct) {
  var poolState = await getPoolState();

  const max_exchange_rate = Math.round(poolState['token_eth_rate'] * (1 + maxSlippagePct / 100) * decimilization);
  const min_exchange_rate = Math.round(poolState['token_eth_rate'] * (1 - maxSlippagePct / 100) * decimilization);


  await token_contract.connect(provider.getSigner(defaultAccount)).approve(defaultAccount, amountEth);
  return await exchange_contract.connect(provider.getSigner(defaultAccount)).addLiquidity(max_exchange_rate, min_exchange_rate, { value: ethers.utils.parseUnits(amountEth.toString(), 'wei') });
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  const max_exchange_rate = Math.floor(poolState['eth_token_rate'] * (1 + maxSlippagePct / 100) * decimilization);
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
  ...
  const max_exchange_rate = Math.floor(poolState['token_eth_rate'] * (1 + maxSlippagePct / 100) * decimilization); // maybe send in with a fixed denom
